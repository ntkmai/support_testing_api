...
export class FileExplorer {
    ...
    // Load file structure from api-docs folder
    async loadFileStructure() {
        // Get current project ID from localStorage
        const projectId = localStorage.getItem('current_project_id') || 'ke-toan';
        const basePath = `api-docs/${projectId}`;
        ...
        try {
            ...
            // Check if manifest has folders array
            if (!manifest.folders || !Array.isArray(manifest.folders) || manifest.folders.length === 0) {
                console.warn('Manifest does not have folders array');
                UIComponents.showNotification('Project chưa có cấu trúc thư mục', 'warning');
                return;
            }
